...
def experiment_propagation_delay_timetagger(
    ...
    **kwargs,
    ...
    tagger_ch1 = 1
    if vp > 0: tagger_ch2 = 2
    else:      tagger_ch2 = -2


    global vp_polarity

    # Account for negative pulses by changing the AWG waveform
    if (vp_polarity == 'vp+') and vp < 0:
        print('Switching to negative polarity')
        awgpulse.load_file('temp2.wfm')
        time.sleep(0.1)
        vp_polarity ='vp-'
    elif (vp_polarity == 'vp-') and vp >= 0:
        print('Switching to positive polarity')
        awgpulse.load_file('temp.wfm')
        time.sleep(0.1)
        vp_polarity = 'vp+'
    ...
    # Account for negative biases
    if vbias < 0:
        awgsin.set_polarity(positive = False)
        tagger_ch2 = -tagger_ch2 # Make the trigger falling-edge
        tagger_ch2_trigger = -tagger_ch2_trigger # Make the trigger negative too
    else:
        awgsin.set_polarity(positive = True)
...
parameter_dict = dict(
    ...
    vbias = [2,-2],
    rbias = 100e3,
    vp = np.hstack([np.linspace(-2,-0.2,21), np.linspace(2,0.2,21)]),
    t = 2e-9,
    att_db = 0,
    count_time = 0.25,
    vp_splitter = True,
...
# Save data
filename = datetime.datetime.now().strftime('%Y-%m-%d %H-%M-%S propagation_delay_timetagger ')
df = pd.DataFrame(data_list)
df.to_csv(filename + '.csv')

#%%
for name, gd in df.groupby(['ibias','vp_polarity']):
    plt.semilogx(abs(gd.vp), gd.t_median, marker = '.', label = 'Ibias=%0.1fuA, %s' % (name[0]*1e6, name[1]))
plt.legend()
plt.title('Effect of pulse/bias polarity on A17A19\n100kΩ bias resistor')
plt.xlabel('Power (W)')
plt.ylabel('Propagation delay (ps)')
# df.groupby('ibias').plot('vp','t_median', marker = '.', ax=plt.gca())

#%%
df.plot('tagger_ch2_trigger','t_median', marker = '.')
