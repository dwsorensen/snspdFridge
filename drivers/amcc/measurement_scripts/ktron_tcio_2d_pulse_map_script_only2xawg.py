...
def plot_2d_energy_vs_bias(df, max_count_ratio = 2.1):
    # Plot 2D (Pulse voltage)  vs (Pulse length), where color = vdmm (latching or not)
    dfall = df
    for ports, df2 in dfall.groupby('ports'):
...
def experiment_count_tron_clicks(
    t = 50e-9,
    vbias = 0.5,
    vpulse = 0.1,
    vpulse_att_db = 40,
...
#    pulses_per_sine = 1,
    ...
    **kwargs,
    ...
    awg.set_clock(1/t)
#    awg.set_vhighlow(vlow = 0, vhigh = vpulse)
    awg.set_vpp(abs(vpulse))
    awg.set_voffset(vpulse/2)
#    awgsin.set_vhighlow(vlow = 0, vhigh = vbias/2)
    ...
    pulse_count_ratio = counts/(count_time*pulse_rate)
    ...
    data = dict(
        ...
        ibias = ibias,
        vpulse = vpulse,
        vpulse_att_db = vpulse_att_db,
        rbias = rbias,
        count_time = count_time,
        pulse_rate = pulse_rate,
        ports = ports,
        counts = counts,
        pulse_count_ratio = pulse_count_ratio,
        energy_in = energy_in
        )
...
# - Double-check that trigger set to "External" on pulse-generating-AWG
# - Set phase on sine-bias-AWG to "90" and double-check marker lines up with sine wave
...
channel_data = np.zeros(512)
channel_data[10] = 1
marker_data = np.zeros(512)
marker_data[10:] = 1

awg.create_waveform(voltages = channel_data, filename = 'temp.wfm', clock = None, marker1_data = marker_data, auto_fix_sample_length = False)
awg.load_file('temp.wfm')
awg.set_vhighlow(vlow = 0, vhigh = 1)
awg.set_marker_vhighlow(vlow = 0, vhigh = 1)
...
# Run the experiment
df = run_experiment(
    experiment_fun = experiment_count_tron_clicks,
    parameter_dict = parameter_dict,
    )


# Save data
filename = datetime.datetime.now().strftime('%Y-%m-%d %H-%M-%S experiment_count_tron_clicks')
df.to_csv(filename +  '.csv')


# Plot
plot_2d_energy_map(df)
