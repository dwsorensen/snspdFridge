...
class LeCroy620Zi(object):
    ...
    def set_sample_mode(self, sequence = True, num_segments = 10):
        if sequence is True:
            self.vbs_write('app.Acquisition.Horizontal.SampleMode = "Sequence"')
            self.vbs_write(f'app.Acquisition.Horizontal.NumSegments = {num_segments}')
        else:
            self.vbs_write('app.Acquisition.Horizontal.SampleMode = "Realtime"')
    ...
    def get_wf_data(self,channel='C1', out_of_range_as_nan = True, downsample = 1):  # e.g. channel = C1 or F3 etc
    
        def downsample_and_average(arr, n):
            end =  n * int(len(arr)/n)
            return np.mean(arr[:end].reshape(-1, n), 1)
        
        # If in sequence mode, stop the trigger so it doesn't hang
        sequence = False
        if self.vbs_ask('app.Acquisition.Horizontal.SampleMode') == 'Sequence':
        ...
        time.sleep(0.1) # Pause here briefly - can timeout if read happens too fast
        databytes = self.read_raw()
        
        # If stopped trigger because in sequence mode, set trigger mode back
        if sequence is True: self.set_trigger_mode(trigger_mode)
        ...
        usertext_length = int(np.frombuffer(databytes[40:40+4], np.int32))
        trigtime_length = int(np.frombuffer(databytes[48:48+4], np.int32))
        risetime_length = int(np.frombuffer(databytes[52:52+4], np.int32))
        
        wavedesc = databytes[:wavedesc_length] # 'WAVEDESC' block
        databytes = databytes[wavedesc_length+trigtime_length+risetime_length:]
        trigtime = databytes[wavedesc_length:wavedesc_length+trigtime_length]
        ...
        ADDR_FIRST_VALID_PNT = 124
        ADDR_LAST_VALID_PNT = 128
        ADDR_SEGMENT_INDEX = 140
        ADDR_SUBARRAY_COUNT = 144
        vgain = float(np.frombuffer(wavedesc[ADDR_VGAIN:ADDR_VGAIN+4], np.float32))
        voffset = float(np.frombuffer(wavedesc[ADDR_VOFFSET:ADDR_VOFFSET+4], np.float32))
        hinterval = float(np.frombuffer(wavedesc[ADDR_HINTERVAL:ADDR_HINTERVAL+4], np.float32))
        hoffset = float(np.frombuffer(wavedesc[ADDR_HOFFSET:ADDR_HOFFSET+8], np.double))
        hinterval = float(np.frombuffer(wavedesc[ADDR_HINTERVAL:ADDR_HINTERVAL+4], np.float32))
        hoffset = float(np.frombuffer(wavedesc[ADDR_HOFFSET:ADDR_HOFFSET+8], np.double))
        last_valid_pnt = int(np.frombuffer(wavedesc[ADDR_LAST_VALID_PNT:ADDR_LAST_VALID_PNT+4], np.int32))
        first_valid_pnt = int(np.frombuffer(wavedesc[ADDR_FIRST_VALID_PNT:ADDR_FIRST_VALID_PNT+4], np.int32))
        segment_index = int(np.frombuffer(wavedesc[ADDR_SEGMENT_INDEX:ADDR_SEGMENT_INDEX+4], np.int32))
        subarray_count = int(np.frombuffer(wavedesc[ADDR_SUBARRAY_COUNT:ADDR_SUBARRAY_COUNT+4], np.int32))
        num_datapoints_array = int(np.frombuffer(wavedesc[ADDR_WAVE_ARRAY_COUNT:ADDR_WAVE_ARRAY_COUNT+4], np.int32))
        num_points_scope = int(np.frombuffer(wavedesc[ADDR_PNTS_PER_SCREEN:ADDR_PNTS_PER_SCREEN+4], np.int32))
        ...
        if subarray_count > 1:
            x = np.split(x, subarray_count)
            y = np.split(y, subarray_count)
            # Clip to # of points on scope
            for n in range(subarray_count):
                x[n] = x[n][:int(num_points_scope/subarray_count)]
                y[n] = y[n][:int(num_points_scope/subarray_count)]
            # Adjust time-offset so all segments overlap
            x0 = x[0][0]
            ...
            if downsample > 1:
                for n in range(subarray_count):
                    x[n] = downsample_and_average(x[n], downsample)
                    y[n] = downsample_and_average(y[n], downsample)
        else:
            ...
            if downsample > 1:
                x = downsample_and_average(x, downsample)
                y = downsample_and_average(y, downsample)
